...
def hd_bet(
    ...
    if output_file_or_dir is None:
        output_file_or_dir = os.path.join(
            os.path.dirname(input_file_or_dir),
    ...
    assert os.path.abspath(input_file_or_dir) != os.path.abspath(output_file_or_dir), (
    ...
    if overwrite_existing == 0:
        ...
        raise ValueError(
            "Unknown value for overwrite_existing: %s. Expected: 0 or 1" % str(overwrite_existing)
        )
    ...
    run_hd_bet(
        input_files, output_files, mode, config_file, device, pp, tta, save_mask, overwrite_existing
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="input. Can be either a single file name or an input folder. If file: must be "
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="output. Can be either a filename or a folder. If it does not exist, the folder"
        " will be created",
        required=False,
        type=str,
    )
    parser.add_argument(
        ...
        type=str,
        default="accurate",
        help="can be either 'fast' or 'accurate'. Fast will "
    ...
    parser.add_argument(
        ...
        required=False,
        type=int,
        help="whether to use test time data augmentation "
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="set to 0 to disabe postprocessing (remove all"
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="if set to 0 the segmentation mask will not be saved",
    )
    parser.add_argument(
        "--overwrite_existing",
        default=1,
        type=int,
        required=False,
        help="set this to 0 if you don't want to overwrite existing predictions",
    )
