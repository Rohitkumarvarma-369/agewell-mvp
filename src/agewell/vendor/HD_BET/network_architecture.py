...
class EncodingModule(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv3d(
            in_channels, out_channels, filter_size, 1, (filter_size - 1) // 2, bias=self.conv_bias
        )
        ...
        self.conv2 = nn.Conv3d(
            out_channels, out_channels, filter_size, 1, (filter_size - 1) // 2, bias=self.conv_bias
        )
...
class LocalizationModule(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(
            self.bn_1(self.conv1(x)), negative_slope=self.leakiness, inplace=self.lrelu_inplace
        )
        x = F.leaky_relu(
            self.bn_2(self.conv2(x)), negative_slope=self.leakiness, inplace=self.lrelu_inplace
        )
        return x
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(
            self.bn_after_context5(x), negative_slope=self.leakiness, inplace=self.lrelu_inplace
        )
        x = self.up1(x)
